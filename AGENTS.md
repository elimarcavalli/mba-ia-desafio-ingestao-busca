...
- **`src/domain/`** — Pure business logic, NO external imports (no LangChain, no SQLAlchemy). Contains entities (`Document`, `DocumentChunk`, `SearchResult`, `User`), abstract port interfaces (ABCs for `RepositoryPort`, `LLMPort`, `EmbeddingsPort`, `DocumentLoaderPort`, `UserRepositoryPort`, `PasswordHasherPort`), and domain exceptions.
- **`src/application/use_cases/`** — Orchestrates ports. Three use cases: `IngestDocumentUseCase` (document → chunks → embeddings → DB), `SearchDocumentsUseCase` (query → retrieval → LLM → answer), and `AuthenticateOrRegisterUserUseCase` (sign-in or auto sign-up on first login).
...
The Chainlit web UI uses a **single-form sign-in / sign-up** pattern: an unknown username at the login screen creates a new account; a known one verifies the password (Argon2id hash). This is implemented through ports (`UserRepositoryPort`, `PasswordHasherPort`) and the `AuthenticateOrRegisterUserUseCase`, mirroring the rest of the architecture. Sessions are scoped per user — Chainlit's data layer filters threads by `userIdentifier`. The only required env var is `CHAINLIT_AUTH_SECRET` (auto-generated by `main.py`); credentials live in the `User` table, never in `.env`.
...
| `CHAINLIT_AUTH_SECRET` | env var               | required by Chainlit to sign session JWTs; generated by `main.py` |
...
- **RAG tuning env vars:** `CHUNK_SIZE` (default 1000), `CHUNK_OVERLAP` (default 150), `RETRIEVER_K` (default 10), `LLM_TIMEOUT` (default 60s)
